...
import {
    ...
    SportsFootball, SportsBasketball, SportsTennis, SportsHockey, SportsVolleyball
...
export default function SingleEventManager() {
    ...
    const handleParticipantStatus = async (participantId, accepted) => {
        try {
            ...
            showSnackbar(`Participant status ${accepted ? 'approved' : 'rejected'}`, 'success');
        } catch (error) {
            showSnackbar(error.response?.data?.message || error.message || 'Update failed', 'error');
        }
    };
                                            ...
                                            participants.map((participant) => (
                                                <ParticipantCard key={participant.user.id} elevation={1}>
                                                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                        ...
                                                        <Box>
                                                            <Tooltip title={participant.accepted ? "Revoke acceptance" : "Accept participant"}>
                                                                <IconButton
