...
const mockTeams = [
    {
        ...
        nextEvent: "Match vs Red Tigers, May 12"
    },
    {
        id: 2,
        ...
        country: "Canada",
        city: "Toronto",
        achievements: "League Finalist 2023",
        wins: 9,
        members: 15,
        logo: "/api/placeholder/100/100",
        nextEvent: "Practice, May 10"
    },
    {
        id: 3,
        teamName: "Silver Wolves",
        teamType: "HOCKEY",
        status: "Inactive team",
        country: "Russia",
        city: "Moscow",
        achievements: "City Tournament Winner 2022",
        wins: 5,
        members: 22,
        logo: "/api/placeholder/100/100",
        nextEvent: "None"
                    ...
                    subheader={
                        <Box sx={{display: 'flex', alignItems: 'center', mt: 0.5}}>
                            <Chip
                                ...
                                label={team.teamType}
                                sx={{mr: 1}}
                                color={team.status === "Active team" ? "success" : "default"}
                            />
                            <Chip
                                size="small"
...
export const teamDataSource = {
    ...
    deleteOne: async (teamId) => {
        try {
            await fetch(`${TEAM_API_URL}/${teamId}`, {
                method: "DELETE",
                headers: getHeaders(),
            });
        } catch (error) {
...
function TeamManagementModern() {
    ...
    const teamMembers = teams.filter(member => member.teamId === selectedTeamId);
    ...
    const toggleAddMemberDialog = async () => {
        if (!isAddMemberOpen) {
            // Загружаем доступных пользователей при открытии диалога
            try {
    ...
    const handleAddMember = async () => {
        try {
            const response = await fetch(`${TEAM_API_URL}/member`, {
                method: "POST",
                headers: getHeaders(),
            ...
            if (response.ok) {
                ...
                console.error("Ошибка при добавлении участника:", await response.text());
            }
        } catch (error) {
    ...
    const fetchTeamMembers = async (teamId) => {
        try {
            ...
            const response = await fetch(`${TEAM_API_URL}/members`, {
                method: "GET",
                headers: getHeaders(),
            });
            ...
            // Фильтруем участников только для выбранной команды
            const teamMembers = memberData.filter(member => member.teamId === teamId);
    ...
    const handleRemoveMember = async (memberId) => {
        try {
            const response = await fetch(`${TEAM_API_URL}/member?id=${memberId}&teamId=${selectedTeamId}`, {
                method: "DELETE",
                headers: getHeaders(),
            });
            ...
            if (response.ok) {
                ...
                console.error("Ошибка при удалении участника:", await response.text());
            }
        } catch (error) {
        ...
        const fetchTeamsAndMembers = async () => {
            try {
                ...
                const formattedTeams = teamData.map(team => ({
                    ...
                    status: team.status || "Inactive team",
                    country: team.country,
                    city: team.city,
                    achievements: team.achievements || "",
                    wins: team.wins || "0",
                    members: 0,  // Будет обновлено, когда получим участников
                    logo: "/api/placeholder/100/100",
                ...
                if (formattedTeams.length > 0) {
                    ...
                    // Загружаем участников для первой команды
    ...
    const handleCreateTeam = async () => {
        try {
            ...
            if (response.ok) {
                ...
                const teamsResponse = await fetch(TEAM_API_URL, {
                    method: "GET",
                    headers: getHeaders(),
                });
                ...
                // Выбираем новую команду, если это первая созданная команда
                if (formattedTeams.length > 0 && teams.length === 0) {
                ...
                console.error("Ошибка при создании команды:", await response.text());
            }
        } catch (error) {
    ...
    const handleUpdateTeam = async () => {
        try {
            ...
            if (response.ok) {
                ...
                console.error("Ошибка при обновлении команды:", await response.text());
            }
        } catch (error) {
        ...
        <AppTheme>
            ...
            <Box>
                ...
                >
                    ...
                    {/* Динамическое содержимое на основе состояния загрузки */}
                    {renderContent()}
                </Box>
                            ...
                            <Grid container spacing={2}>
                                ...
                                <Grid item xs={12} sm={6}>
                                    <TextField
                                        ...
                                        onChange={handleCreateTeamChange}
                                    >
                                        <MenuItem value="Active team">Active team</MenuItem>
                                        <MenuItem value="Inactive team">Inactive team</MenuItem>
                                    </TextField>
                                </Grid>
                                <Grid item xs={12} sm={6}>
                                    <TextField
