package com.sporteventstournaments.security.repository;
...
public interface SecurityCredentialsRepository extends JpaRepository<SecurityCredentials, Long> {
    ...
    @Query("SELECT u FROM security_credentials u WHERE u.verificationCode = ?1")
    Optional<SecurityCredentials>  findByVerificationCode(String code);

    @Query(value = "SELECT NEXTVAL('security_credentials_id_seq')", nativeQuery = true)
