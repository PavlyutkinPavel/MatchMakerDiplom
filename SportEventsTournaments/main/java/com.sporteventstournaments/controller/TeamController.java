...
public class TeamController {
    ...
    public ResponseEntity<HttpStatus> deleteTeam(@PathVariable Long id, Principal principal) {
        ...
        } /*else if (securityService.checkIfCreatorOrDirector(principal.getName())) {
            teamService.deleteTeamById(id);
            return new ResponseEntity<>(HttpStatus.NO_CONTENT);
        } */else {
   ...
   /* @Operation(summary = "get all Teams(for all)")
    ...
    @Operation(summary = "update Team (for authorized users)")
    @PutMapping("/member")
    public ResponseEntity<HttpStatus> updateTeamMember(@RequestBody Team team, Principal principal) {
        if(principal == null){
            return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
        }
        return teamService.updateTeamMember(team, principal);
    }

    @Operation(summary = "delete Team (for authorized users)")
    @DeleteMapping("/{id}")
    public ResponseEntity<HttpStatus> deleteTeamMember(@PathVariable Long id, Principal principal) {
        if(principal == null){
            return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
        }
        if (securityService.checkIfAdmin(principal.getName())) {
